...
SELECT pf.*, ps.nombre, ps.apellido1, ps.apellido2, dp.nombre as departamento FROM profesor as pf LEFT JOIN persona as ps ON pf.id_profesor = ps.id LEFT JOIN departamento as dp ON pf.id_departamento = dp.id;
...
SELECT asg.nombre, c.anyo_inicio, c.anyo_fin FROM alumno_se_matricula_asignatura as a LEFT JOIN persona as p ON a.id_alumno = p.id LEFT JOIN asignatura as asg ON a.id_asignatura = asg.id LEFT JOIN curso_escolar as c ON a.id_curso_escolar = c.id where p.nif="26902806M";
...
SELECT d.*, p.id_profesor as profesor, a.nombre as asignatura, g.nombre as grado FROM departamento as d LEFT JOIN profesor as p ON d.id = p.id_departamento LEFT JOIN asignatura as a on p.id_profesor = a.id_profesor LEFT JOIN grado as g ON a.id_grado = g.id WHERE g.nombre="Grado en Ingeniería Informática (Plan 2015)";

-- 9. Retorna un llistat amb tots els alumnes que s'han matriculat en alguna assignatura durant el curs escolar 2018/2019.
SELECT DISTINCT(a.id_alumno) as alumne, p.nombre, p.apellido1, p.apellido2, c.* FROM alumno_se_matricula_asignatura as a LEFT JOIN persona as p ON a.id_alumno = p.id LEFT JOIN curso_escolar as c ON a.id_curso_escolar = c.id WHERE anyo_inicio = '2018';
